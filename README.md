...
#### 4.4.2 验证表单数据（编写校验删除文章分类的id）

#### 4.4.3 实现删除文章功能

### 4.5 根据 ID 获取文章分类

#### 4.5.1 定义根据id获取文章分类的路由和路由处理函数
...
#### 4.6.1 定义根据id更新文章分类的路由和路由处理函数

#### 4.6.2 验证表单数据

#### 4.6.3 查询分类与分类别名是否被占用

1. 定义查重 sql,首先排除用户传入的id那条数据，然后从非当前id数据中 判断是否有名称或别名被占用，如果查询出有数据，则证明被占用
